...
mkValidator beneficiary deadline () ctx = traceIfFalse "beneficiary's signature missing" checkSig &&
    ...
    where
      ...
      checkSig :: Bool
      checkSig = txSignedBy info beneficiary
        
      checkDeadline :: Bool 
      checkDeadline = contains (from deadline) $ txInfoValidRange info
