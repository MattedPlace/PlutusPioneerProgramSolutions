...
payContract2 = do 
    Contract.handleError (\err -> Contract.logError $ "caught error: " ++ unpack err) payContract1
    payContract2
-- A trace that invokes the pay endpoint of payContract on Wallet 1 twice, each time with Wallet 2 as
-- recipient, but with amounts given by the two arguments. There should be a delay of one slot
-- after each endpoint call.
payTrace :: Integer -> Integer -> EmulatorTrace ()
payTrace payAmount1 payAmount2 = do
    h1 <- activateContractWallet (Wallet 1) payContract2
    h2 <- activateContractWallet (Wallet 2) payContract2
    callEndpoint @"pay" h1 $ PayParams
        { ppRecipient = pubKeyHash $ walletPubKey $ Wallet 2
        , ppLovelace  = payAmount1
        }
    void $ Emulator.waitNSlots 1
    callEndpoint @"pay" h1 $ PayParams
        { ppRecipient = pubKeyHash $ walletPubKey $ Wallet 2
        , ppLovelace  = payAmount2
        }
    void $ Emulator.waitNSlots 1
